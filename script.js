...
const class_body_search = /(export class SocietyUnitsComponent implements OnInit \{\s*)([\s\S]*?)(\s*constructor)/;
const class_body_replacement = `$1
    wings: Wing[] = [];
...
$3`;
content = content.replace(class_body_search, class_body_replacement);

const methods_search = /(ngOnInit\(\): void \{)([\s\S]*?)(get filteredFlats\(\): Flat\[\] \{[\s\S]*?\n      \})/;
const methods_replacement = `$1
...
`;
content = content.replace(methods_search, methods_replacement);
