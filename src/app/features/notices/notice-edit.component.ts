...
import { NoticeUpdateRequest, Notice } from '../../core/models/notice.model';
...
@Component({
    ...
    <div class="create-notice-container animate-up">
      <header class="page-header">
        <div class="header-content">
          ...
          <h1>Edit <span>Notice</span></h1>
          <p>Update existing announcement for the society</p>
        </div>
      </header>
      ...
      <div class="form-card glass-card">
        ...
        <form [formGroup]="noticeForm" (ngSubmit)="onSubmit()" *ngIf="!isLoading">
          ...
          <div class="form-row">
            <div class="form-group full-width">
              ...
              <div class="file-upload-container" [class.has-file]="selectedFile || existingAttachment">
                <input type="file" id="attachment" (change)="onFileSelected($event)" accept=".pdf,.png,.jpg,.jpeg,.webp" class="file-input">
                <div class="upload-placeholder" *ngIf="!selectedFile && !existingAttachment">
                  <i class="fas fa-cloud-upload-alt"></i>
                  <p>Click to upload or drag and drop</p>
                  <span>Replace existing file (PDF, PNG, JPG up to 10MB)</span>
                </div>
                <div class="file-info" *ngIf="selectedFile">
                  <i class="fas fa-file-alt"></i>
                  <span class="filename">{{ selectedFile.name }} (New)</span>
                  <button type="button" class="remove-btn" (click)="removeFile()"><i class="fas fa-times"></i></button>
                </div>
                <div class="file-info" *ngIf="!selectedFile && existingAttachment">
                  <i class="fas fa-file-alt"></i>
                  <span class="filename">{{ existingAttachment.name }} (Current)</span>
                  <button type="button" class="remove-btn" (click)="removeExistingAttachment()"><i class="fas fa-times"></i></button>
                </div>
              </div>
            </div>
          </div>
  ...
  styles: [`
    ...
    .loading-state { text-align: center; padding: 3rem; font-size: 1.25rem; color: #64748b; i { font-size: 2rem; margin-right: 1rem; color: #2563eb; } }
...
export class NoticeEditComponent implements OnInit {
  ...
  loadNoticeDetails(): void {
    this.noticeService.getNoticeDetails(this.noticeId).subscribe({
      next: (notice) => {
        if (notice) {
          ...
          this.noticeForm.patchValue({
            ...
            targetFlatNumber: notice.targetFlatNumber || ''
          });
      ...
      error: () => {
        this.notificationService.error('Failed to load notice');
        this.router.navigate(['/notices']);
      }
    });
  }
  ...
  private saveNotice(): void {
    const request: NoticeUpdateRequest = {
      ...
      active: this.originalNotice.active,
    ...
    this.noticeService.updateNotice(request).subscribe({
      next: () => {
        this.notificationService.success('Notice updated successfully');
        this.router.navigate(['/notices']);
      },
      error: () => {
        this.notificationService.error('Failed to update notice');
        this.isSubmitting = false;
      }
    });
  }
